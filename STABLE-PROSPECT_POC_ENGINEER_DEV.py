...
def add_prospect_to_staging(prospect_data, selected_contact=None):
    ...
    # Debug: Let's see what data we're getting from prospect_data
    print(f"DEBUG: Staging prospect {prospect_data.get('DBA_NAME', 'Unknown')}")
    print(f"  Original prospect_data keys: {list(prospect_data.keys())}")
    print(f"  ADDRESS: '{prospect_data.get('ADDRESS', 'MISSING')}'")
    print(f"  CITY: '{prospect_data.get('CITY', 'MISSING')}'")
    print(f"  STATE: '{prospect_data.get('STATE', 'MISSING')}'")
    print(f"  ZIP: '{prospect_data.get('ZIP', 'MISSING')}'")
    print(f"  ZI_C_LOCATION_ID: '{prospect_data.get('ZI_C_LOCATION_ID', 'MISSING')}'")
    ...
    # Debug: Let's see what's actually in our staging record
    print(f"DEBUG: Staged record created:")
    print(f"  original_address: '{staging_record.get('original_address', 'MISSING')}'")
    print(f"  original_city: '{staging_record.get('original_city', 'MISSING')}'")
    print(f"  original_state: '{staging_record.get('original_state', 'MISSING')}'")
    print(f"  original_zip: '{staging_record.get('original_zip', 'MISSING')}'")
    print(f"  zi_c_location_id: '{staging_record.get('zi_c_location_id', 'MISSING')}'")
...
def validate_staged_prospect_enhanced(prospect):
    ...
    # Check if this prospect needs contact selection
    prospect_id = prospect.get('prospect_id')
    needs_contact_selection = False
    contact_selection_valid = True
    
    if prospect_id and hasattr(st.session_state, 'filtered_df') and not st.session_state.filtered_df.empty:
        matching_rows = st.session_state.filtered_df[
            st.session_state.filtered_df['PROSPECT_ID'] == prospect_id
        ]
        if not matching_rows.empty:
            original_prospect_data = matching_rows.iloc[0]
            top_contacts = original_prospect_data.get("TOP10_CONTACTS")
            ...
            main_table_contact = {
                "name": original_prospect_data.get("CONTACT_NAME", ""),
                "email_address": original_prospect_data.get("CONTACT_EMAIL", ""),
                "direct_phone_number": original_prospect_data.get("CONTACT_PHONE", ""),
                "mobile_phone": original_prospect_data.get("CONTACT_MOBILE", ""),
                "job_title": original_prospect_data.get("CONTACT_JOB_TITLE", ""),
                "source": "main_table"
            }
                    ...
                    if len(selected_for_sf) > 0:
                        ...
                        with button_col1:
                            if all_pushed:
                                ...
                                success_msg = f'<p style="color:#0c8a15; font-size:11px; margin:0; padding:0; font-weight:500;">✓ All sent to Salesforce</p>'
        ...
        if staged_prospects:
            ...
            for i, prospect in enumerate(staged_prospects):
                ...
                with st.expander(f"{status_emoji} {company_name} - {status_text}", expanded=False):
                    ...
                    # Try to get the original prospect data to access TOP10_CONTACTS
                    # We need to match this staged prospect back to the filtered data
                    original_prospect_data = None
                    if hasattr(st.session_state, 'filtered_df') and not st.session_state.filtered_df.empty:
                        # Find the matching prospect in filtered_df by prospect_id
                        matching_rows = st.session_state.filtered_df[
                            st.session_state.filtered_df['PROSPECT_ID'] == prospect_id
                        ]
                        if not matching_rows.empty:
                    ...
                    # Parse TOP10_CONTACTS if available
                    if original_prospect_data is not None:
                        top_contacts = original_prospect_data.get("TOP10_CONTACTS")
                    ...
                    # Add main table contact as an option if it has meaningful data and is not already in TOP10_CONTACTS
                    main_table_contact = {
                        "name": original_prospect_data.get("CONTACT_NAME", "") if original_prospect_data is not None else "",
                        "email_address": original_prospect_data.get("CONTACT_EMAIL", "") if original_prospect_data is not None else "",
                        "direct_phone_number": original_prospect_data.get("CONTACT_PHONE", "") if original_prospect_data is not None else "",
                        "mobile_phone": original_prospect_data.get("CONTACT_MOBILE", "") if original_prospect_data is not None else "",
                        "job_title": original_prospect_data.get("CONTACT_JOB_TITLE", "") if original_prospect_data is not None else "",
                        "source": "main_table"
                    }
                    
                    # Check if main table contact has meaningful data
                    main_contact_has_data = any(
                        is_meaningful_value(main_table_contact.get(field))
                        for field in ["name", "email_address", "direct_phone_number", "mobile_phone", "job_title"]
                    )
                    ...
                    def is_similar_contact(contact1, contact2):
                        ...
                        # Check email first (most reliable identifier)
                        email1 = contact1.get("email_address", "").strip().lower()
                        email2 = contact2.get("email_address", "").strip().lower()
                        if email1 and email2 and email1 == email2:
                            return True
                        
                        # Check name similarity
                        name1 = contact1.get("name", "").strip().lower()
                        name2 = contact2.get("name", "").strip().lower()
                        
                        # Check phone similarity
                        phone1 = (contact1.get("direct_phone_number", "") or contact1.get("mobile_phone", "")).strip()
                        phone2 = (contact2.get("direct_phone_number", "") or contact2.get("mobile_phone", "")).strip()
                        
                        # Consider similar if name and phone match (and both have values)
                        if name1 and name2 and phone1 and phone2:
                            return name1 == name2 and phone1 == phone2
                        
                        return False
                    
                    # Check if main table contact already exists in TOP10_CONTACTS
                    main_contact_already_exists = False
                    if main_contact_has_data:
                        for existing_contact in contacts_available:
                            if is_similar_contact(main_table_contact, existing_contact):
                                main_contact_already_exists = True
                                break
                    
                    # Only add main table contact if it has data and doesn't already exist
                    if main_contact_has_data and not main_contact_already_exists:
                        contacts_available.insert(0, main_table_contact)  # Add at beginning
                    ...
                    if len(contacts_available) > 1:
                        ...
                        for idx, contact in enumerate(contacts_available):
                            ...
                            source_indicator = " (Current)" if contact.get("source") == "main_table" else ""
