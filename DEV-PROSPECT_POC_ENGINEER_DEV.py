...
def escape_sql_number(value):
    """
    Escape a numeric value for SQL, handling None and various formats.
    Returns 'NULL' for None values.
    """
...
def insert_prospect_to_sf_table(prospect_id, session):

    try:
        # First, get the prospect data from the main table
        prospect_query = f"""
        SELECT 
            prospect_id,
            address,
        ...
        prospect_data = prospect_result[0]
        ...
        VALUES (
            ...
            {escape_sql_string(insert_data["no_of_employees"])},
...
def insert_prospect_to_sf_table_from_staging(staged_prospect, session):
    """
    Insert a prospect to sf_leads table using data from staging (with selected contact)
    """
    try:
        prospect_id = staged_prospect.get('prospect_id')
        ...
        # Use the contact data from staging (which includes selected contact info)
        first_name = staged_prospect.get('first_name', '') or None
        last_name = staged_prospect.get('last_name', '') or None
        
        # Prepare the data for insertion using staging data
        insert_data = {
            'id': f"{prospect_id}_{owner_id}",  # Concatenate prospect_id and owner_id for the ID field
            'address': staged_prospect.get('address'),
            'historical_affiliate_id': None,  # Always null per mapping
            'annual_revenue': staged_prospect.get('revenue'),
            'city': staged_prospect.get('city'),
            'state': staged_prospect.get('state'),
            'zip': staged_prospect.get('zip'),
            'company': staged_prospect.get('company'),
            'company_industry': staged_prospect.get('industry'),
            'email': staged_prospect.get('email'),
            'federal_tax_id': staged_prospect.get('taxid'),
            'first_name': first_name,
            'last_name': last_name,
            'lead_affiliate_ids': None,  # Always null per mapping
            'mcc_code_lookup': staged_prospect.get('mcc_code'),
            'lead_mid': staged_prospect.get('identifier'),
            'no_of_locations': staged_prospect.get('number_of_locations'),
            'no_of_employees': staged_prospect.get('employees'),
            'phone': staged_prospect.get('phone'),
            'phone_mobile': staged_prospect.get('contact_mobile'),
            'phone_extension': None,  # Always null per mapping
            'sic_code': None,  # Always null per mapping
            'sic_description': None,  # Always null per mapping
            'vendor_company_id': staged_prospect.get('zi_c_company_id'),
            'vendor_contact_id': staged_prospect.get('zi_contact_id'),
            'website': staged_prospect.get('website'),
            'zi_location_id': staged_prospect.get('data_agg_uid'),
            'data_vendor_names': 'ZoomInfo' if str(staged_prospect.get('data_agg_uid', '')).startswith('Wkkx') else None,
            'lead_source_url': 'www.globalpayments.com',
            'owner_id': owner_id,
            'record_type_id': None,  # Always null per mapping
            'lead_source': 'Self Sourced',
            'lead_source_desc': 'GP Prospecting Portal',
            'status': '1a. Cold Prospecting',
            'title': staged_prospect.get('job_title'),
            'zi_location_id_org': staged_prospect.get('zi_c_location_id')
        }
...
def insert_prospect_to_sf_table_from_staging_debug(staged_prospect, session):
    """
    Enhanced debug version of insert function with detailed logging
    """
    try:
        prospect_id = staged_prospect.get('prospect_id')
        
        # Debug: Log what we're trying to insert
        st.write(f"🔍 DEBUG: Attempting to insert prospect_id: {prospect_id}")
        st.write(f"🔍 DEBUG: Staged prospect keys: {list(staged_prospect.keys())}")
        
        # Get owner_id from Salesforce user table based on email
        streamlit_user_email = get_streamlit_user_email()
        st.write(f"🔍 DEBUG: Streamlit user email: {streamlit_user_email}")
        
        owner_lookup = get_salesforce_owner_id(streamlit_user_email, session)
        
        if not owner_lookup['success']:
            st.error(f"❌ DEBUG: Owner lookup failed: {owner_lookup['message']}")
            return {
                'success': False,
                'message': f"Owner lookup failed: {owner_lookup['message']}",
                'is_duplicate': False,
                'detailed_error': f"Could not find Salesforce owner for email: {streamlit_user_email}"
            }
        
        owner_id = owner_lookup['owner_id']
        st.write(f"✅ DEBUG: Found owner_id: {owner_id}")
        
        # Use the contact data from staging (which includes selected contact info)
        first_name = staged_prospect.get('first_name', '') or None
        last_name = staged_prospect.get('last_name', '') or None
        
        st.write(f"🔍 DEBUG: Contact info - First: '{first_name}', Last: '{last_name}'")
        st.write(f"🔍 DEBUG: Email: '{staged_prospect.get('email')}'")
        st.write(f"🔍 DEBUG: Company: '{staged_prospect.get('company')}'")
        
        # Prepare the data for insertion using staging data
        insert_data = {
            'id': f"{prospect_id}_{owner_id}",
            'address': staged_prospect.get('address'),
            'historical_affiliate_id': None,
            'annual_revenue': staged_prospect.get('revenue'),
            'city': staged_prospect.get('city'),
            'state': staged_prospect.get('state'),
            'zip': staged_prospect.get('zip'),
        ...
        # Clean up revenue field - remove $ and commas, convert to number
        if insert_data['annual_revenue']:
            revenue_str = str(insert_data['annual_revenue'])
            # Remove $ and commas, keep only digits and decimal point
            cleaned_revenue = ''.join(c for c in revenue_str if c.isdigit() or c == '.')
            try:
                insert_data['annual_revenue'] = float(cleaned_revenue) if cleaned_revenue else None
            except ValueError:
                insert_data['annual_revenue'] = None
        
        st.write(f"✅ DEBUG: Insert data prepared for company: {insert_data['company']}")
        st.write(f"🔍 DEBUG: Generated ID: {insert_data['id']}")
        st.write(f"🔍 DEBUG: Cleaned revenue: {insert_data['annual_revenue']}")
        
        # Show some key fields that might be causing issues
        problematic_fields = []
        if not insert_data['company']:
            problematic_fields.append("company (empty)")
        if not insert_data['first_name'] and not insert_data['last_name']:
            problematic_fields.append("first_name and last_name (both empty)")
        if not insert_data['email']:
            problematic_fields.append("email (empty)")
            
        if problematic_fields:
            st.warning(f"⚠️ DEBUG: Potential issues with fields: {', '.join(problematic_fields)}")
        ...
        st.write(f"🔍 DEBUG: Checking for duplicates with ID: {insert_data['id']}")
        
        try:
            duplicate_result = session.sql(duplicate_check_query).collect()
            existing_count = duplicate_result[0]['COUNT_EXISTING']
            st.write(f"🔍 DEBUG: Duplicate check result: {existing_count} existing records")
            
            if existing_count > 0:
                st.warning(f"⚠️ DEBUG: Duplicate found - record already exists")
                return {
                    'success': False,
                    'message': f'Duplicate record: {insert_data["company"] or "Unknown"} already exists in Salesforce',
                    'is_duplicate': True,
                    'detailed_error': f'Record with ID {insert_data["id"]} already exists'
                }
        except Exception as dup_error:
            st.error(f"❌ DEBUG: Duplicate check failed: {str(dup_error)}")
            return {
                'success': False,
                'message': f'Error checking for duplicates: {str(dup_error)}',
                'is_duplicate': False,
                'detailed_error': f'Duplicate check query failed: {str(dup_error)}'
            }
        
        # Build the INSERT statement with proper formatting
        insert_query = f"""
        INSERT INTO sandbox.conklin.sf_leads (
            id, address, historical_affiliate_id, annual_revenue, city, state, zip,
            company, company_industry, email, federal_tax_id, first_name, last_name,
            lead_affiliate_ids, mcc_code_lookup, lead_mid, no_of_locations, no_of_employees,
            phone, phone_mobile, phone_extension, sic_code, sic_description,
            vendor_company_id, vendor_contact_id, website, zi_location_id, data_vendor_names,
            lead_source_url, owner_id, record_type_id, lead_source, lead_source_desc,
            status, title, zi_location_id_org
        )
        VALUES (
            {escape_sql_string(insert_data["id"])},
            {escape_sql_string(insert_data["address"])},
        ...
        st.write("🔍 DEBUG: About to execute INSERT query")
        
        # Execute the insert
        try:
            session.sql(insert_query).collect()
            st.success("✅ DEBUG: INSERT query executed successfully!")
            ...
            st.error(f"❌ DEBUG: Database error occurred: {error_str}")
            
            # Also show the problematic query for debugging
            with st.expander("🔍 DEBUG: Show INSERT Query"):
            ...
            if "duplicate key" in error_str.lower() or "unique constraint" in error_str.lower():
                return {
                    'success': False,
                    'message': f'Duplicate record: {insert_data["company"] or "prospect"} already exists in Salesforce',
                    'is_duplicate': True,
                    'detailed_error': error_str
                }
            elif "null value" in error_str.lower() or "cannot be null" in error_str.lower():
                return {
                    'success': False,
                    'message': f'Missing required data for {insert_data["company"] or "prospect"}. A required database field is null.',
                    'is_duplicate': False,
                    'detailed_error': error_str
                }
            elif "invalid data" in error_str.lower() or "data type" in error_str.lower():
                return {
                    'success': False,
                    'message': f'Invalid data format for {insert_data["company"] or "prospect"}. Check data types and formats.',
                    'is_duplicate': False,
                    'detailed_error': error_str
                }
            elif "table" in error_str.lower() and "not found" in error_str.lower():
                return {
                    'success': False,
                    'message': f'Database table access error. Please contact administrator.',
                    'is_duplicate': False,
                    'detailed_error': error_str
                }
            else:
                return {
                    'success': False,
                    'message': f'Database error inserting {insert_data["company"] or "prospect"}: {error_str}',
                    'is_duplicate': False,
                    'detailed_error': error_str
                }
        ...
        error_str = str(e)
        st.error(f"❌ DEBUG: General error occurred: {error_str}")
        
        # Add context about what was being attempted
        context_info = []
        if staged_prospect.get('prospect_id'):
            context_info.append(f"Prospect ID: {staged_prospect.get('prospect_id')}")
        if staged_prospect.get('company'):
            context_info.append(f"Company: {staged_prospect.get('company')}")
        if staged_prospect.get('first_name') or staged_prospect.get('last_name'):
            contact_name = f"{staged_prospect.get('first_name', '')} {staged_prospect.get('last_name', '')}".strip()
            context_info.append(f"Contact: {contact_name}")
        
        context_str = " | ".join(context_info) if context_info else "No context available"
        
        return {
            'success': False,
            'message': f'Error processing prospect ({context_str}): {error_str}',
            'is_duplicate': False,
            'detailed_error': error_str,
            'context': context_str
        }
...
def add_prospect_to_staging(prospect_data, selected_contact=None):
    ...
    existing_ids = [p.get("prospect_id") for p in st.session_state.staged_prospects]
    if prospect_id in existing_ids:
        return False  # Already staged
    
    # Parse TOP10_CONTACTS to get available contacts
    import json
    ...
    if top_contacts:
        ...
        if isinstance(contacts_obj, dict) and bool(contacts_obj):
            available_contacts = list(contacts_obj.values())
        elif isinstance(contacts_obj, list) and len(contacts_obj) > 0:
            available_contacts = contacts_obj
    
    # Use selected contact if provided, otherwise use default contact info from main table
    if selected_contact and available_contacts:
        ...
        # Fallback to main table contact info
    ...
    staging_record = {
        ...
        "address": prospect_data.get("ADDRESS", ""),  # Street address component
        "full_address": prospect_data.get("FULL_ADDRESS", ""),  # Keep full address for reference
        ...
        "has_multiple_contacts": len(available_contacts) > 1,
        "contact_selection_required": len(available_contacts) > 1 and not selected_contact
    }
...
def remove_prospect_from_staging(prospect_id):
    ...
    return len(st.session_state.staged_prospects) < original_count


def update_contact_selection(prospect_id, selected_contact):
    """Update the selected contact for a staged prospect"""
    if "staged_prospects" not in st.session_state:
        return False
    ...
    for prospect in st.session_state.staged_prospects:
        if prospect.get("prospect_id") == prospect_id:
            ...
            if selected_contact:
                prospect["selected_contact"] = selected_contact
                prospect["contact_selection_required"] = False
                
                # Update contact fields in the staging record
                prospect["contact_name"] = selected_contact.get("name", "")
                prospect["email"] = selected_contact.get("email_address", "")
                prospect["contact_phone"] = selected_contact.get("direct_phone_number", "")
                prospect["contact_mobile"] = selected_contact.get("mobile_phone", "")
                prospect["job_title"] = selected_contact.get("job_title", "")
                
                # Update first/last name
                name_parts = prospect["contact_name"].split(' ') if prospect["contact_name"] else ['', '']
                prospect["first_name"] = name_parts[0] if len(name_parts) > 0 else ""
                prospect["last_name"] = name_parts[-1] if len(name_parts) > 1 else ""
...
def validate_staged_prospect(prospect):
    ...
    # Check if contact selection is required but not completed
    if prospect.get("contact_selection_required", False):
...
def submit_staged_prospects_to_sf():
    ...
    for prospect in st.session_state.staged_prospects:
        ...
        # Submit to sf_leads table (pass the staged prospect data with selected contact)
        result = insert_prospect_to_sf_table_from_staging_debug(prospect, session)
                ...
                if 'Map' in edited_df.columns and 'SF' in edited_df.columns:
                    ...
                    if len(selected_for_sf) > 0:
                        ...
                        for _, prospect in selected_for_sf.iterrows():
                            ...
                            prospect_id_str = str(prospect_id)
                            if prospect_id_str not in get_sf_prospect_ids():
                                all_pushed = False
                                break
                        ...
                        with button_col1:
                            if all_pushed:
                                ...
                                success_msg = f'<p style="color:#0c8a15; font-size:11px; margin:0; padding:0; font-weight:500;">✓ All written</p>'
                                ...
                                if st.button(button_label, type="primary", key="sf_push_button"):
                                    # Stage prospects instead of directly writing to sf_leads
                                    staged_count = 0
                                    skipped_count = 0
                                    
                                    for _, prospect in selected_for_sf.iterrows():
                                        if add_prospect_to_staging(prospect):
                                            staged_count += 1
                                        else:
                                            skipped_count += 1
                        ...
                        if len(st.session_state.selected_prospect_indices) == 1:
                            ...
                            if not already_pushed:
                                ...
                                with sf_cols[0]:  # Button in left column
                                    ...
                                    if push_button:
                                        ...
                                        if add_prospect_to_staging(prospect_data):
                                            success_msg = f'<p style="color:#0c8a15; font-size:11px; margin:0; padding:0; font-weight:500;">✅ {prospect_name} staged for Salesforce</p>'
                                            st.markdown(success_msg, unsafe_allow_html=True)
                                            st.info("💡 Go to the Salesforce tab to review and submit")
                                        else:
                                            warning_msg = f'<p style="color:#ff8c00; font-size:11px; margin:0; padding:0; font-weight:500;">⚠️ {prospect_name} already staged</p>'
                                            st.markdown(warning_msg, unsafe_allow_html=True)
                                        
                                        # Log for debugging
                                        print(f"Processed prospect ID {prospect_id}: {result['message']}")
                            ...
                            with btn_col1:
                                if all_pushed:
                                    ...
                                    st.markdown('<p style="color:#0c8a15; font-size:11px; margin:0; padding:0; font-weight:500;">✓ All written</p>', unsafe_allow_html=True)
                                else:
        ...
        if staged_prospects:
            ...
            for i, prospect in enumerate(staged_prospects):
                ...
                with st.expander(f"{status_emoji} {company_name} - {status_text}", expanded=False):
                    ...
                    available_contacts = prospect.get('available_contacts', [])
                    has_multiple_contacts = prospect.get('has_multiple_contacts', False)
                    selected_contact = prospect.get('selected_contact')
                    
                    if has_multiple_contacts and available_contacts:
                        st.markdown("**📋 Contact Selection** *(Required)*")
                        st.markdown("*This prospect has multiple contacts available. Please select which contact information to use for the Salesforce lead.*")
                        
                        # Create contact options for dropdown
                        contact_options = []
                        contact_map = {}
                        
                        # Add default "Select a contact" option
                        contact_options.append("Select a contact...")
                        
                        for idx, contact in enumerate(available_contacts):
                            name = contact.get('name', 'Unknown')
                            title = contact.get('job_title', '')
                            email = contact.get('email_address', '')
                            phone = contact.get('direct_phone_number', '') or contact.get('mobile_phone', '')
                            
                            # Create display string with better formatting
                            ...
                            option_text = " ".join(display_parts)
                            contact_options.append(option_text)
                            contact_map[option_text] = contact
                        
                        # Determine current selection
                        current_selection = "Select a contact..."
                        if selected_contact:
                            for option, contact in contact_map.items():
                                if contact.get('name') == selected_contact.get('name'):
                                    current_selection = option
                                    break
                        
                        # Contact selection dropdown with enhanced styling
                        st.markdown("""
                        <style>
                        ...
                        selected_option = st.selectbox(
                            "Choose contact for Salesforce:",
                            options=contact_options,
                            index=contact_options.index(current_selection) if current_selection in contact_options else 0,
                            key=f"contact_select_{prospect_id}",
                            help="Select which contact information to use when creating the Salesforce lead"
                        )
                        
                        # Update selection if changed
                        if selected_option != "Select a contact..." and selected_option != current_selection:
                            new_contact = contact_map.get(selected_option)
                            if new_contact:
                                update_contact_selection(prospect_id, new_contact)
                                st.rerun()
                        
                        # Show validation feedback with enhanced styling
                        if selected_option == "Select a contact...":
                            st.error("❌ **Contact selection required** - Please select a contact before submitting to Salesforce")
                        else:
                            selected_contact_name = selected_contact.get('name', 'Unknown') if selected_contact else 'None'
                            selected_contact_title = selected_contact.get('job_title', '') if selected_contact else ''
                            
                            feedback_text = f"✅ **Contact selected:** {selected_contact_name}"
                            if selected_contact_title:
                                feedback_text += f" ({selected_contact_title})"
                            
                            st.success(feedback_text)
                        
                        st.markdown("---")
                    
                    elif available_contacts and len(available_contacts) == 1:
                        # Auto-selected single contact
                        st.markdown("**📋 Contact Information**")
                        single_contact = available_contacts[0]
                        contact_name = single_contact.get('name', 'Unknown')
                        contact_title = single_contact.get('job_title', '')
                        contact_email = single_contact.get('email_address', '')
                        ...
                        st.success(f"✅ Auto-selected contact: {contact_info}")
                        st.markdown("*This prospect has only one contact available, so it was automatically selected.*")
                        st.markdown("---")
                    ...
                    # Contact Information
                    contact_details = []
                    if is_meaningful_value(prospect.get('first_name')):
                        contact_details.append(f"**First Name:** {prospect['first_name']}")
                    if is_meaningful_value(prospect.get('last_name')):
                        contact_details.append(f"**Last Name:** {prospect['last_name']}")
                    if is_meaningful_value(prospect.get('email')):
                        contact_details.append(f"**Email:** {prospect['email']}")
                    if is_meaningful_value(prospect.get('phone')):
                        contact_details.append(f"**Phone:** {prospect['phone']}")
                    if is_meaningful_value(prospect.get('contact_mobile')):
                        contact_details.append(f"**Mobile:** {prospect['contact_mobile']}")
                    if is_meaningful_value(prospect.get('job_title')):
                        contact_details.append(f"**Job Title:** {prospect['job_title']}")
                    ...
                    if any([contact_details, company_details, location_details]):
                        ...
                        if contact_details:
                            ...
                            st.dataframe(contact_df, hide_index=True, use_container_width=True)
                            st.markdown("")
                                ...
                                if result['success']:
                                    ...
                                    with message_container:
                                        ...
                                        if result.get('duplicate_count', 0) > 0:
                                            ...
                                            # Show detailed results in an expander for debugging
                                            if result.get('results'):
                                                with st.expander("🔍 View detailed error information"):
                                                    for res in result['results']:
                                                        if res['status'] == 'error':
                                                            st.write(f"**{res['prospect']}**: {res['message']}")
                                                            if 'detailed_error' in res:
                                                                st.code(res['detailed_error'])
                                            
                                            # Show summary statistics
                                            st.write(f"Total processed: {result.get('total_processed', 0)}")
                                            st.write(f"Errors: {result.get('error_count', 0)}")
                                            st.write(f"Duplicates: {result.get('duplicate_count', 0)}")
                                            st.write(f"Successes: {result.get('success_count', 0)}")
                            except Exception as e:
                                with message_container:
            ...
            with col3:
                ...
                valid_prospects = 0
                for prospect in staged_prospects:
                    if validate_staged_prospect(prospect)['is_valid']:
                        valid_prospects += 1
