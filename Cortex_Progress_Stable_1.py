...
def main():
    ...
    if apply_filters and has_active_filters(filters):
        def fetch_data():
            ...
            reset_to_first_page()
            # Fetch ALL data for client-side pagination to avoid empty page issues
        ...
        st.rerun()
    with tab1:
        if hasattr(st.session_state, 'filtered_df') and hasattr(st.session_state, 'total_records') and not st.session_state.filtered_df.empty and st.session_state.active_filters:
            if "limit_warning" in st.session_state:
            ...
            display_df = st.session_state.filtered_df.iloc[start_idx:end_idx].drop(columns=['LONGITUDE', 'LATITUDE'], errors='ignore')
