...
def insert_prospect_to_sf_table(prospect_id, session):
    """
    Insert a prospect record into the sf_leads Snowflake table.
    
    Args:
        prospect_id: The prospect ID to insert
        session: Active Snowflake session
    ...
    try:
        ...
        if duplicate_result[0]['COUNT_EXISTING'] > 0:
            return {
                'success': False,
                'message': f'Prospect {insert_data["company"] or "Unknown"} already exists in sf_leads table',
                'is_duplicate': True
            }
        ...
        return {
            'success': True,
            'message': f'Successfully added {insert_data["company"] or "prospect"} to sf_leads table',
            'is_duplicate': False
        }
        ...
        return {
            'success': False,
            'message': f'Error inserting prospect to sf_leads: {str(e)}',
            'is_duplicate': False
        }


def test_sf_insert_function(test_prospect_id=None):
    """
    Test function for the sf_leads insert functionality.
    Can be called from the UI for testing.
    """
    try:
        # Use a test prospect ID or get one from the current data
        if not test_prospect_id:
            test_query = f"SELECT prospect_id FROM {get_table_name()} LIMIT 1"
            test_result = session.sql(test_query).collect()
            if test_result:
                test_prospect_id = test_result[0]['PROSPECT_ID']
            else:
                return "No prospects found in source table for testing"
        
        # Test owner lookup first
        user_email = get_streamlit_user_email()
        owner_lookup = get_salesforce_owner_id(user_email, session)
        
        if not owner_lookup['success']:
            return f"Owner lookup failed: {owner_lookup['message']}"
        
        # Test the insert function
        result = insert_prospect_to_sf_table(test_prospect_id, session)
        
        return f"Test Result: {result['message']} (Success: {result['success']}, Duplicate: {result['is_duplicate']}) | Owner ID: {owner_lookup['owner_id']} for {user_email}"
...
def add_prospect_to_salesforce(prospect_id):
    """
    Add a prospect to Salesforce by inserting into sf_leads table.
    
    Args:
        prospect_id: The prospect ID to add
        
    Returns:
        dict: {'success': bool, 'message': str, 'is_duplicate': bool, 'already_tracked': bool}
    """
    ...
    # Attempt to insert the prospect into the sf_leads table
    try:
...
def add_prospect_to_staging(prospect_data, selected_contact=None):
    """Add a prospect to the staging area for Salesforce submission with contact selection"""
    ...
    # Get contact info (selected or auto-selected)
    if not selected_contact:
        selected_contact = smart_contact_selection(prospect_data, "staging")
    ...
    # Create staging record with selected contact
    staging_record = {
        ...
        "contact_mobile": selected_contact.get("mobile", ""),
        "contact_phone": selected_contact.get("phone", ""),  # Selected contact's direct phone
        "address": prospect_data.get("FULL_ADDRESS") or prospect_data.get("ADDRESS", ""),
        "website": prospect_data.get("WEBSITE", ""),
        "job_title": selected_contact.get("title", ""),
        ...
        # Store selected contact info for reference
        "selected_contact": selected_contact
    }
    
    # Parse contact name into first and last name
    contact_name = staging_record.get("contact_name", "")
    if contact_name:
        name_parts = contact_name.split(' ')
        staging_record["first_name"] = name_parts[0] if len(name_parts) > 0 else ""
        staging_record["last_name"] = name_parts[-1] if len(name_parts) > 1 else ""
...
def submit_staged_prospects_to_sf():
    ...
    for prospect in st.session_state.staged_prospects:
        ...
        # Submit to sf_leads table
        result = insert_prospect_to_sf_table(prospect["prospect_id"], session)
...
def select_contact_for_salesforce(prospect_data, prospect_key=None):
    """Display contact selector and return selected contact info"""
    import json
    
    top_contacts = prospect_data.get("TOP10_CONTACTS")
    if not top_contacts:
        # Use fallback contact from main table
        return {
            "name": prospect_data.get("CONTACT_NAME", ""),
            "email": prospect_data.get("CONTACT_EMAIL", ""),
            "phone": prospect_data.get("CONTACT_PHONE", ""),
            "mobile": prospect_data.get("CONTACT_MOBILE", ""),
            "title": prospect_data.get("CONTACT_JOB_TITLE", "")
        }
    
    # Parse contacts
    try:
        contacts_obj = json.loads(top_contacts) if isinstance(top_contacts, str) else top_contacts
    ...
    if not contacts_obj or len(contacts_obj) == 0:
        # No contacts available, use fallback
        return {
            "name": prospect_data.get("CONTACT_NAME", ""),
            "email": prospect_data.get("CONTACT_EMAIL", ""),
            "phone": prospect_data.get("CONTACT_PHONE", ""),
    ...
    if len(contacts_obj) == 1:
        # Only one contact, use it automatically
        contact = list(contacts_obj.values())[0]
        return format_contact_for_sf(contact)
    
    # Multiple contacts - show selector
    contact_options = []
    contact_map = {}
    
    # Sort contacts by rank (1, 2, 3, etc.)
    sorted_contacts = sorted(contacts_obj.items(), key=lambda x: int(x[0]) if x[0].isdigit() else 999)
    
    for rank, contact in sorted_contacts:
        ...
        # Add email if available for clarity
        if email:
            label += f" • {email}"
        
        contact_options.append(label)
        contact_map[label] = contact
    
    # Generate unique key for this selector
    prospect_id = prospect_data.get("PROSPECT_ID") or prospect_data.get("IDENTIFIER", "unknown")
    selector_key = f"contact_select_{prospect_id}_{prospect_key}" if prospect_key else f"contact_select_{prospect_id}"
    
    # Streamlit selector
    selected_label = st.selectbox(
        f"📋 Select contact for {prospect_data.get('DBA_NAME', 'this company')}:",
        contact_options,
        key=selector_key,
        help="Choose which contact to send to Salesforce"
    )
    
    return format_contact_for_sf(contact_map[selected_label])

def format_contact_for_sf(contact):
    """Format contact for Salesforce insertion"""
    return {
        "name": contact.get('name', ''),
        "email": contact.get('email_address', ''),
        "phone": contact.get('direct_phone_number', ''),
        "mobile": contact.get('mobile_phone', ''),
        "title": contact.get('job_title', '')
    }

def get_fallback_contact(prospect_data):
    """Get fallback contact info from main table"""
    return {
        "name": prospect_data.get("CONTACT_NAME", ""),
        "email": prospect_data.get("CONTACT_EMAIL", ""),
        "phone": prospect_data.get("CONTACT_PHONE", ""),
...
def smart_contact_selection(prospect_data, prospect_key=None):
    """Automatically select best contact with override option"""
    import json
    
    top_contacts = prospect_data.get("TOP10_CONTACTS")
    if not top_contacts:
        return get_fallback_contact(prospect_data)
    ...
    # Auto-select the top-ranked contact (usually rank "1")
    top_contact = contacts_obj.get("1") or list(contacts_obj.values())[0]
    
    # Generate unique keys
    prospect_id = prospect_data.get("PROSPECT_ID") or prospect_data.get("IDENTIFIER", "unknown")
    change_key = f"change_contact_{prospect_id}_{prospect_key}" if prospect_key else f"change_contact_{prospect_id}"
    show_key = f"show_selector_{prospect_id}_{prospect_key}" if prospect_key else f"show_selector_{prospect_id}"
    
    # Show auto-selection with option to change
    col1, col2 = st.columns([3, 1])
    
    with col1:
        name = top_contact.get('name', 'Unknown')
        title = top_contact.get('job_title', '')
        st.info(f"**Auto-selected:** {name} - {title}")
    
    with col2:
        if st.button("Change Contact", key=change_key):
            st.session_state[show_key] = True
    
    # Show full selector if requested
    if st.session_state.get(show_key, False):
        return select_contact_for_salesforce(prospect_data, prospect_key)
    
    return format_contact_for_sf(top_contact)

def bulk_contact_selection_and_staging(selected_prospects_df):
    """Handle contact selection for multiple prospects and add to staging"""
    
    contact_selections = {}
    staged_count = 0
    
    st.write("### 📋 Contact Selection for Salesforce")
    st.write("Select which contact to send to Salesforce for each company:")
    
    for idx, prospect in selected_prospects_df.iterrows():
        ...
        with st.expander(f"📍 {company_name}", expanded=False):
            # Show prospect context
            col1, col2 = st.columns([2, 1])
            with col1:
                st.write(f"**Company:** {company_name}")
                if prospect.get('PRIMARY_INDUSTRY'):
                    st.write(f"**Industry:** {prospect.get('PRIMARY_INDUSTRY')}")
                if prospect.get('REVENUE'):
                    try:
                        revenue = float(prospect.get('REVENUE'))
                        st.write(f"**Revenue:** ${revenue:,.0f}")
                    except (ValueError, TypeError):
                        pass
            
            with col2:
                if prospect.get('NUMBER_OF_EMPLOYEES'):
                    st.write(f"**Employees:** {prospect.get('NUMBER_OF_EMPLOYEES')}")
                if prospect.get('CITY') and prospect.get('STATE'):
                    st.write(f"**Location:** {prospect.get('CITY')}, {prospect.get('STATE')}")
            
            # Contact selection
            selected_contact = select_contact_for_salesforce(prospect, f"bulk_{idx}")
            contact_selections[prospect_id] = selected_contact
            
            # Add to staging button
            if st.button(f"✅ Add {company_name} to Staging", key=f"stage_{prospect_id}_{idx}"):
                success = add_prospect_to_staging(prospect, selected_contact)
                if success:
                    st.success(f"Added {company_name} to staging area!")
                    staged_count += 1
                else:
                    st.warning(f"{company_name} is already in staging area.")
    
    if staged_count > 0:
        st.success(f"Successfully staged {staged_count} prospects for Salesforce!")
    
    return contact_selections

def add_prospect_to_salesforce_with_contact_selection(prospect_data):
    """Add a single prospect to Salesforce with contact selection"""
    st.markdown("### 📋 Select Contact for Salesforce")
    
    # Show prospect context
    col1, col2 = st.columns([2, 1])
    with col1:
        st.write(f"**Company:** {prospect_data.get('DBA_NAME', 'Unknown')}")
        if prospect_data.get('PRIMARY_INDUSTRY'):
            st.write(f"**Industry:** {prospect_data.get('PRIMARY_INDUSTRY')}")
    
    with col2:
        if prospect_data.get('REVENUE'):
            try:
                revenue = float(prospect_data.get('REVENUE'))
                st.write(f"**Revenue:** ${revenue:,.0f}")
            except (ValueError, TypeError):
                pass
    ...
    # Contact selection
    prospect_id = prospect_data.get('PROSPECT_ID') or prospect_data.get('IDENTIFIER')
    selected_contact = select_contact_for_salesforce(prospect_data, f"single_{prospect_id}")
    
    # Add to staging with selected contact
    if st.button("✅ Add to Salesforce Queue", key=f"add_single_{prospect_id}", type="primary"):
        if add_prospect_to_staging(prospect_data, selected_contact):
            st.success("Added to Salesforce staging area!")
            st.info("💡 Go to the Salesforce tab to review and submit")
        else:
            st.warning("This prospect is already in the staging area.")
...
def create_data_editor_column_config():
    ...
    return {
        ...
        "SF": st.column_config.CheckboxColumn("SF", help="Check to write to sf_leads table", default=False, width="small"),
    ...
    if 'sf_bulk_result' in st.session_state:
        ...
        if result['newly_added'] > 0 or result['duplicates'] > 0 or result['errors'] > 0:
            if result['newly_added'] > 0:
                st.success(f"✅ Successfully added {result['newly_added']} prospects to sf_leads table")
            ...
            def load_data_editor():
                ...
                if 'Map' in edited_df.columns and 'SF' in edited_df.columns:
                    ...
                    if len(selected_for_sf) > 0:
                        st.caption(f"🚀 {len(selected_for_sf)} prospectes selected for sf_leads table")
                        ...
                        with button_col1:
                            if all_pushed:
                                ...
                                button_label = "Stage Selected for Salesforce"
                                ...
                                if st.button(button_label, type="primary", key="sf_push_button"):
                                    ...
                                    if staged_count > 0:
                                        if skipped_count > 0:
                                            st.success(f"✅ Staged {staged_count} prospects for Salesforce ({skipped_count} already staged)")
                                        else:
                                            st.success(f"✅ Staged {staged_count} prospects for Salesforce")
                                        st.info("💡 Go to the Salesforce tab to review and submit")
                                    else:
                        ...
                        if len(st.session_state.selected_prospect_indices) == 1:
                            ...
                            if not already_pushed:
                                ...
                                with sf_cols[0]:  # Button in left column
                                    ...
                                    button_label = f"Stage {prospect_name} for Salesforce"
                            ...
                            with btn_col1:
                                if all_pushed:
                                    ...
                                    button_label = "Write Selected to sf_leads Table"
                                    ...
                                    if bulk_push:
                                        ...
                                        # Add each prospect to Salesforce
                                        result = add_prospectes_to_salesforce(selected_prospectes)
                                        
                                        # Store the result for display after rerun
                                        st.session_state.sf_bulk_result = result
    ...
    with tab3:
        ...
        <div style="padding: 20px; border-radius: 12px; background: linear-gradient(135deg, #f8faff 0%, #ffffff 100%); 
            ...
            <h2 style="color: #262aff; margin-bottom: 20px; display: flex; align-items: center;">
                <span style="font-size: 28px; margin-right: 10px;">🚀</span> Salesforce Staging
            </h2>
            <p style="font-size: 16px; color: #333; margin-bottom: 20px;">
                Review and submit prospects that have been staged for Salesforce integration.
            </p>
        </div>
        """, unsafe_allow_html=True)
        ...
        # Summary row
        if staged_prospects:
            st.success(f"✅ {len(staged_prospects)} prospects staged for Salesforce submission")
        else:
            st.info("No prospects currently staged. Use the 'Stage Selected for Salesforce' button in List View or Map View to add prospects.")
        ...
        if staged_prospects:
            ...
            # Display staged prospects table
            st.subheader("Staged Prospects")
            ...
            with col1:
                if st.button("🚀 Submit All to sf_leads Table", type="primary", key="submit_all_staged"):
                    ...
                    if validation_errors:
                        ...
                        with st.spinner("Submitting prospects to sf_leads table..."):
                            try:
                                ...
                                if result['success']:
                                    ...
                                    clear_staging()
                                    with message_container:
                                        st.success(f"✅ Successfully submitted {result['success_count']} prospects to sf_leads table!")
            ...
            with col2:
                if st.button("🗑️ Clear All Staging", type="secondary", key="clear_all_staging"):
                    clear_staging()
                    with message_container:
                        st.success("✅ Staging cleared successfully")
                    st.rerun()
            ...
            with col3:
                ...
                if valid_prospects == total_prospects:
                    st.success(f"✅ {valid_prospects}/{total_prospects} prospects valid")
                else:
                    st.warning(f"⚠️ {valid_prospects}/{total_prospects} prospects valid")
            
            # Individual prospect management
            st.subheader("Individual Prospect Management")
            
            # Create expandable sections for each prospect
            for i, prospect in enumerate(staged_prospects):
                prospect_id = prospect.get('prospect_id', f'prospect_{i}')
                company_name = prospect.get('company', 'Unknown Company')
                
                # Validation status for this prospect
                validation_result = validate_staged_prospect(prospect)
                status_icon = "✅" if validation_result['is_valid'] else "❌"
                
                with st.expander(f"{status_icon} {company_name} (ID: {prospect_id})", expanded=False):
                    # Display validation status
                    if validation_result['is_valid']:
                        st.success("✅ All required fields present")
                    else:
                        st.error(f"❌ Missing required fields: {', '.join(validation_result['errors'])}")
                    
                    # Display prospect fields in columns
                    col_left, col_right = st.columns(2)
                    
                    with col_left:
                        st.write("**Company Information:**")
                        st.write(f"Company: {prospect.get('company', 'N/A')}")
                        st.write(f"Industry: {prospect.get('industry', 'N/A')}")
                        st.write(f"Revenue: {prospect.get('revenue', 'N/A')}")
                        st.write(f"Employees: {prospect.get('employees', 'N/A')}")
                        
                        st.write("**Contact Information:**")
                        st.write(f"First Name: {prospect.get('first_name', 'N/A')}")
                        st.write(f"Last Name: {prospect.get('last_name', 'N/A')}")
                        st.write(f"Title: {prospect.get('job_title', 'N/A')}")
                        st.write(f"Email: {prospect.get('email', 'N/A')}")
                        st.write(f"Phone: {prospect.get('phone', 'N/A')}")
                    
                    with col_right:
                        st.write("**Address Information:**")
                        st.write(f"Address: {prospect.get('address', 'N/A')}")
                        st.write(f"City: {prospect.get('city', 'N/A')}")
                        st.write(f"State: {prospect.get('state', 'N/A')}")
                        st.write(f"ZIP: {prospect.get('zip', 'N/A')}")
                        
                        st.write("**Additional Information:**")
                        st.write(f"Website: {prospect.get('website', 'N/A')}")
                        st.write(f"Mobile: {prospect.get('contact_mobile', 'N/A')}")
                        st.write(f"Tax ID: {prospect.get('taxid', 'N/A')}")
                        st.write(f"MCC Code: {prospect.get('mcc_code', 'N/A')}")
                    
                    # Individual remove button
                    if st.button(f"🗑️ Remove {company_name}", key=f"remove_prospect_{prospect_id}", type="secondary"):
                        remove_prospect_from_staging(prospect_id)
                        with message_container:
                            st.success(f"✅ Removed {company_name} from staging")
                        st.rerun()
        
        # Configuration and info section at bottom
        st.markdown("---")
        ...
        with info_col1:
            ...
            **Auto-populated Fields:**
            - Lead Status: "1a. Cold Prospecting"
            - Lead Source: "Self Sourced"
            - Lead Source Description: "GP Prospecting Portal"
            """)
        
        with info_col2:
            st.info("""
            **Data Integration:**
            - Target: sandbox.conklin.sf_leads
