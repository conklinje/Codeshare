...
# Height adjustment JavaScript removed - not needed in Streamlit for Snowflake

# Sidebar scaling JavaScript removed - not needed in Streamlit for Snowflake

# Sidebar layout - Option 2: Minimalist Tab-Based Design
if not st.session_state.sidebar_collapsed:
    with st.sidebar:
        ...
        st.markdown("<h1 class='gp-center-text gp-font-sm gp-color-primary' style='margin: 0rem 0; font-family: var(--font-family-primary);'>Prospecting Hub</h1>", unsafe_allow_html=True)
        ...
        st.subheader("Cortex Analyst Prompt")
        sidebar_prompt = st.text_input("Ask a question about your prospects...", value=st.session_state.sidebar_cortex_prompt, key="sidebar_cortex_prompt")
        # The widget manages session state automatically. No need to set st.session_state.sidebar_cortex_prompt here.
        st.markdown("<hr>", unsafe_allow_html=True)
    ...
    if not st.session_state.sidebar_collapsed:
        with st.sidebar:
            ...
            with st.expander("Saved Searches"):
                user_id = st.text_input("User ID", value="default_user", help="Enter your user ID or email", key="user_id_input")
...
def get_data_editor_column_order():
    """Get standardized column order for data editor display"""
    return [
...
def main():
    ...
    with tab4:
        # Cortex Analyst Tab Implementation
        try:
            import _snowflake
        except ImportError:
            st.error("_snowflake module not found. This tab requires Snowflake Cortex integration.")
            st.stop()
        ...
        def reset_cortex_state():
            ...
            st.session_state.cortex_active_suggestion = None
            st.session_state.cortex_warnings = []
            st.session_state.cortex_form_submitted = {}
        ...
        # Use sidebar prompt if present and not already processed
        sidebar_prompt = st.session_state.get("sidebar_cortex_prompt", "")
        if "last_sidebar_cortex_prompt" not in st.session_state:
            st.session_state.last_sidebar_cortex_prompt = ""
        if user_input:
            prompt_to_use = user_input
        elif sidebar_prompt and sidebar_prompt != st.session_state.last_sidebar_cortex_prompt and (len(st.session_state.cortex_messages) == 0 or st.session_state.cortex_messages[-1].get("content", [{}])[0].get("text", "") != sidebar_prompt):
            prompt_to_use = sidebar_prompt
        else:
            prompt_to_use = None
        ...
        if prompt_to_use:
            ...
            new_user_message = {
                "role": "user",
                "content": [{"type": "text", "text": prompt_to_use}],
            }
            st.session_state.cortex_messages.append(new_user_message)
            st.session_state.last_sidebar_cortex_prompt = prompt_to_use
            ...
            with st.chat_message("analyst"):
                with st.spinner("Waiting for Analyst's response..."):
                    time.sleep(1)
                    request_body = {
                        "messages": st.session_state.cortex_messages,
                    ...
                    if resp["status"] < 400:
                        ...
                        error_msg = f"🚨 Analyst API error: {parsed_content.get('message', 'Unknown error')}"
                        analyst_message = {
                            "role": "analyst",
                            "content": [{"type": "text", "text": error_msg}],
                            "request_id": parsed_content.get("request_id", "")
                        }
                    if "warnings" in parsed_content:
                        st.session_state.cortex_warnings = parsed_content["warnings"]
                    st.session_state.cortex_messages.append(analyst_message)
                    st.rerun()
                ...
                for item in content:
                    if item["type"] == "text":
                        ...
                        for suggestion_index, suggestion in enumerate(item["suggestions"]):
                            if st.button(suggestion, key=f"cortex_suggestion_{idx}_{suggestion_index}"):
    ...
    if cortex_df is not None:
        ...
        st.rerun()
    with tab1:
        # Show Cortex Analyst results if present, otherwise filter results
        show_df = None
        show_total = 0
        sidebar_prompt = st.session_state.get("sidebar_cortex_prompt", "")
        cortex_df = None
                    ...
                    for item in msg.get("content", []):
                        if item.get("type") == "sql" and "statement" in item:
                            try:
                                ...
                                st.error(f"Error executing Cortex SQL for List View: {e}")
                            break
                    if cortex_df is not None:
                        break
        if cortex_df is not None:
            show_df = cortex_df
            show_total = cortex_total_records
        elif hasattr(st.session_state, 'filtered_df') and hasattr(st.session_state, 'total_records') and not st.session_state.filtered_df.empty:
            show_df = st.session_state.filtered_df
            show_total = st.session_state.total_records
        if show_df is not None and not show_df.empty:
            ...
            # Dynamic height calculation based on actual results
            # If we have fewer results than page size, size for actual results
            # If we have full page or more, size for the default page size
            ...
            column_order = get_data_editor_column_order()
