create or replace table sandbox.conklin.top_contacts as (
with dnc_list as (
...
stage as (
    ...
    with sort_contacts_zi as (
        select distinct null as identifier,
                ...
                management_level,
                zoominfo_contact_id,
                sandbox.conklin.normalize_phone(direct_phone_number::varchar) as direct_phone_number,
                sandbox.conklin.normalize_phone(mobile_phone::varchar) as mobile_phone,
                case
    ...
    , sort_contacts_mdm as (
        with contact_rank as (
            Select distinct identifier
                ...
                ,case when owner_phone = 'NA' then null else sandbox.conklin.normalize_phone(owner_phone::varchar) end as mobile_phone
            ...
            Select identifier
                ...
                ,null as management_level
                ,case when primary_contact_wrkphone = 'NA' then null else sandbox.conklin.normalize_phone(primary_contact_wrkphone::varchar) end as direct_phone_number
                ,case when primary_contact_cellphone = 'NA' then null else sandbox.conklin.normalize_phone(primary_contact_cellphone::varchar) end as mobile_phone
...
select identifier,
       ...
       object_agg(contact_rank,
           object_construct_keep_null(
               ...
               'contact_id', zoominfo_contact_id
           )
       ) as top10_contacts
from (
    select *,
